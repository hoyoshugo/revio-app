        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setForm((f) => ({ ...f, period_months: parseInt(e.target.value) }))}
              className="w-full bg-gray-800 text-white rounded-lg px-3 py-2 text-sm border border-gray-700 focus:outline-none focus:border-cyan-500"
            />
          </div>
          <div>
          ...
          <div className="flex gap-2 pt-1">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 bg-gray-800 text-gray-300 rounded-lg py-2 text-sm hover:bg-gray-700"
            >
              Cancelar
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex-1 bg-cyan-600 text-white rounded-lg py-2 text-sm hover:bg-cyan-500 disabled:opacity-50"
            >
...
export default function ClientsManager() {
  ...
  async function saveBillingConfig(id, payload) {
    try {
      await saFetch(`/tenants/${id}/billing-config`, {
        method: 'PATCH',
        body: JSON.stringify(payload),
      });
      setMsg('Configuración de billing actualizada');
      setTimeout(() => setMsg(''), 3000);
      await loadTenants();
    } catch (err) {
  ...
  return (
    <div className="space-y-5">
      ...
      {msg && (
        <div className="bg-green-500/15 text-green-400 text-sm rounded-lg px-4 py-2">{msg}</div>
      )}
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[tenant.status] || 'bg-gray-800 text-gray-400'}`}
                      >
                        {STATUS_LABELS[tenant.status] || tenant.status}
                      </span>
                    </div>
                    <div className="text-gray-500 text-xs mt-0.5">
                      {tenant.contact_email} · {tenant.plan_name || 'Sin plan'}
                    </div>
                  </div>
                  <div className="hidden sm:block text-right">
                    ...
                    <div className="text-white text-xs">
                      {tenant.next_payment_at
                        ? new Date(tenant.next_payment_at).toLocaleDateString('es-CO')
                        : '—'}
                    </div>
                  </div>
                  {expanded === tenant.id ? (
                  ...
                  <div className="px-5 pb-4 bg-gray-800/30 space-y-3">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                      <div>
                        ...
                        <br />
                        <span className="text-white">{tenant.contact_name || '—'}</span>
                      </div>
                      <div>
                        <span className="text-gray-500">Teléfono:</span>
                        <br />
                        <span className="text-white">{tenant.contact_phone || '—'}</span>
                      </div>
                      <div>
                        <span className="text-gray-500">Propiedades:</span>
                        <br />
                        <span className="text-white">{tenant.property_count ?? '—'}</span>
                      </div>
                      <div>
                        <span className="text-gray-500">Convs. mes:</span>
                        <br />
                        <span className="text-white">{tenant.conversations_this_month ?? 0}</span>
                      </div>
                      {tenant.trial_ends_at && (
                        <div>
                          ...
                          <span className="text-white">
                            {new Date(tenant.trial_ends_at).toLocaleDateString('es-CO')}
                          </span>
                        </div>
                      )}
                      <div>
                        ...
                        <span
                          className={
                            tenant.onboarding_completed ? 'text-green-400' : 'text-yellow-400'
                          }
                        >
                          {tenant.onboarding_completed ? 'Completo' : 'Pendiente'}
                        </span>
                      </div>
                    </div>
                    ...
                    {/* Billing config — controla BYOK Anthropic + límite mensual */}
                    <div className="bg-gray-800/50 border border-indigo-500/30 rounded-lg px-3 py-3 space-y-2">
                      <div className="text-[11px] uppercase tracking-wide text-indigo-400 font-semibold">
                        Configuración de billing IA
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                        <label className="text-xs text-gray-400 flex items-center gap-2 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={!!tenant.byok_required}
                            onChange={(e) =>
                              saveBillingConfig(tenant.id, { byok_required: e.target.checked })
                            }
                            className="w-4 h-4 accent-indigo-500"
                          />
                          BYOK requerido
                          <span className="text-[10px] text-gray-500">(bloquea agente sin key propia)</span>
                        </label>
                        <label className="text-xs text-gray-400 flex items-center gap-2 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={!!tenant.platform_billing_enabled}
                            onChange={(e) =>
                              saveBillingConfig(tenant.id, {
                                platform_billing_enabled: e.target.checked,
                              })
                            }
                            className="w-4 h-4 accent-indigo-500"
                          />
                          Platform billing habilitado
                          <span className="text-[10px] text-gray-500">(fallback a key Alzio)</span>
                        </label>
                        <label className="text-xs text-gray-400 flex flex-col gap-1">
                          <span>Límite conversaciones/mes</span>
                          <input
                            type="number"
                            defaultValue={tenant.max_conversations_month ?? 1000}
                            onBlur={(e) => {
                              const v = parseInt(e.target.value, 10);
                              if (!isNaN(v) && v !== tenant.max_conversations_month) {
                                saveBillingConfig(tenant.id, { max_conversations_month: v });
                              }
                            }}
